    ...
    <el-tab-pane>
      ...
      <el-form ref="AccountFrom" :model="account" :rules="rules" label-position="left" label-width="0px"
        ...
        <el-form-item style="width:100%;">
          <el-button type="primary" style="width:100%;" @click.prevent="handleLogin" :loading="loading">登录</el-button>
        </el-form-item>
      </el-form>
    </el-tab-pane>
    <el-tab-pane>
            ...
            .then(
              function(result) {
                ...
                if (result && result.code === '0') {
                  ...
                  that.$store.dispatch('SETNAV',result.router)
                  //that.$store.dispatch('isLogin',true)
                  localStorage.setItem("perms",JSON.stringify(result.perms))
                  localStorage.setItem("currentUser",JSON.stringify(result.user))
                  that.$router.push({ path: "/home" }, (onComplete) => {},(onAbort) => {});
                } else {
                  that.$message.error({
                    showClose: true,
